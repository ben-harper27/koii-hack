...
const EmbeddingTask = require('./embedding-task');
...
class CoreLogic {
  ...
  async task(roundNumber) {
    ...
    try {
      ...
      if (submission) {
        ...
        console.log('embedding model initialized', embeddingModel)
        this.embeddingTask = await new EmbeddingTask(submission.toString(), embeddingModel);
        await this.embeddingTask.start();
        return this.embeddingTask.embedding;
      }
    } catch (e) {
  ...
  async fetchSubmission(roundNumber) {
    ...
    try {
      const r = await fetch(`http://localhost:8000/v0/koii/round/${roundNumber}`);
  ...
  validateNode = async (submission_value, round) => {
    let vote;
    if (this.embeddingTask !== null) {
      vote = await this.embeddingTask.validate(submission_value, round);
    } else {
      vote = true;
    }
    return vote;
  ...
  async submitTask(roundNumber) {
    ...
    try {
      ...
      const submission = this.fetchSubmission(roundNumber);
      if (submission) {
        console.log('SUBMISSION', submission);
